...
provision_env() {
  ...
  cat > "$ENV_FILE" <<EOF
# XeloVoice runtime configuration -- generated by install.sh on $(date -u +%FT%TZ)
# Loaded by systemd (EnvironmentFile) and by install/upgrade scripts.
#
# XeloVoice is designed and developed by Xelocorp, and is one of the products of
# Xelocorp. Do not resell or modify this software without official confirmation
# from Xelocorp.
...
[Unit]
Description=XeloVoice Control Console (by Xelocorp)
...
write_credentials() {
  ...
  cat > "$CREDS_FILE" <<EOF
================================================================
 XeloVoice -- Control Console : CREDENTIALS & INSTALL REPORT
 Generated: $(date -u +%FT%TZ)   Host: $(hostname -f 2>/dev/null || hostname)
================================================================
 XeloVoice is designed and developed by Xelocorp, and is one of the
 products of Xelocorp. Do not resell or modify this software without
